  ...
  <div>
    ...
    <TopNavBar/>
      <section class="fruit_section min-vh-100">
        <div class="container">
          <h2 class="custom_heading mt-3">Human Resource <span style="font-weight: 400;font-size: 18px;">{{filterTable.length}} Documents</span></h2>
          <template v-if="filterTable.length === 0">
            <div class="row layout_padding2" style="display: flex; justify-content: center;">
                <p>
                  No Human resource's content uploaded yet.
                </p>
            </div>
          </template>
          <template v-else>
...
export default {
  ...
  methods:{
    getAllPosts(){
            ...
            this.$store.dispatch('getHRPosts')
            .then(response =>{
        ...
        downloadPolicy(filelocation){
              window.open(`${axios.defaults.baseURL}/static/HRPost/${filelocation}`, '_blank');
        }
  },
  computed:{
        filterTable(){
